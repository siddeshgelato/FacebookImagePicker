 ...
 */
class FacebookImageAdapter(val imageSelectedListener: ImageSelectedListener) :
    RecyclerView.Adapter<FacebookImageAdapter.FacebookImageViewHolder>() {

    private var imageList: List<FacebookPicture> = ArrayList()
    var selectedImageList: ArrayList<FacebookPicture> = ArrayList()
    var selectedImage: FacebookPicture? = null
    ...
    override fun onBindViewHolder(holder: FacebookImageViewHolder, position: Int) {
        ...
        holder.bind(facebookPicture, object : FacebookImageViewHolder.ImageClickListener {
            override fun imageClicked(itemView: View, image: FacebookPicture) {
                ...
                imageSelectedListener.imageSelectedListUpdated(1)
                if (selectedImage?.isSelected == true) {
                    selectedImage?.isSelected = false
                    selectedImage = facebookPicture
        ...
        fun bind(
            ...
            if (imagePlaceHolder != null) {
                loadImage(
                    itemView.context,
                    image.previewUrl,
                    itemView.facebook_picture_image,
                    imagePlaceHolder!!
                )
            } else {
            ...
            itemView.facebook_selected_check_image.visibility =
